...
df <- frame
df$Y1 <- kriging$krige_output@data$var1.pred
df$Y1 <- ifelse(df$Y1 < 0, 0, df$Y1)
df$X1 <- df$Y1
df$var1 <- kriging$krige_output@data$var1.var
df$var1 <- ifelse(df$var1 < 0, 0, df$var1)
df$lon <- cc@coords[,1]
df$lat <- cc@coords[,2]
df$ST1 <- spoints2@data$ST1
head(df)

frame1 <- buildFrameSpatial(df=df,
                            id="id",
                            X=c("X1"),
                            Y=c("Y1"),
                            lon=c("lon"),
...
# gamma_sigma_2 <- computeGamma(e=lm_pred$residuals[camp],
...
# Compute heteroscedasticity index
gamma_sigma_3 <- computeGamma(e=lm_2$residuals,
                              x=spoints_samp@data$P1,
                              nbins=6)
gamma_sigma_3
# gamma     sigma  r.square 
